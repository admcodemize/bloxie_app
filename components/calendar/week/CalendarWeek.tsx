...
import { getWeeks, WeeksObjInfoProps } from "@/helpers/DateTime";
...
const CalendarWeek = ({
  ...
  const getWeekByIndex = React.useCallback((
    ...
    if (!isMounted.current) return getWeeks({ now, weeksInPast: 0, weeksInFuture: 0 })[0];
    ...
    try {
      ...
      const [week] = getWeeks({ now: date, weeksInPast: 0, weeksInFuture: 0 });
      ...
      if (isMounted.current && week) {
        ...
        trimCache();
      } return week || getWeeks({ now, weeksInPast: 0, weeksInFuture: 0 })[0];
    } catch (err) {
      /** @description Fallback to current week on any errors */
      captureException(err as Error, "error", true);
      return getWeeks({ now, weeksInPast: 0, weeksInFuture: 0 })[0];
    }
  }, [now, trimCache]);
